...
public class SoundBehavior : MonoBehaviour
{
    ...
    public float[] volumeSub;
    public Vector2[] pitchSub; // if y=0, only use x
    ...
    public GameObject[] soundVoiceGo;
    public SoundVoice[] soundVoice;
    ...
    #region Voice Building
    public void RebuildVoices()
    {
        DestroyVoices(); // WARNING: this destroys every child to this gameObject
        //DestroyVoicesSafely();
        BuildVoices();
    }
    private void DestroyVoicesSafely()
    {
        if (soundVoiceGo != null)
        {
            for (int i = 0; i < soundVoiceGo.Length; i++)
            {
                DestroyImmediate(soundVoiceGo[i]);
            }
        }
    }
    ...
    {
        soundVoiceGo = new GameObject[voices];
        soundVoice = new SoundVoice[voices];
        for (int i = 0; i < voices; i++)
        {
            soundVoiceGo[i] = new GameObject();
            soundVoiceGo[i].transform.parent = transform;
            soundVoiceGo[i].name = "voice" + i;
            soundVoice[i] = soundVoiceGo[i].AddComponent<SoundVoice>();
        }
    }
    #endregion
    ...
    public void TriggerSpecificSoundVariant(int selectedVariant)
    {
        ...
        if (kill == Kill.Last)
        {
            if (soundVoice[currentVoice].IsPlaying())
                return;
        }
    ...
    public void TriggerSoundObject()
    {
        SelectNextVoice();
        if (kill == Kill.Last)
        {
